...
//		UmengUpdateAgent.setUpdateOnlyWifi(false);
		...
		fbReader.addAction(ActionCode.FEI_WO, new FeiwoAction(this, fbReader));
		fbReader.addAction(ActionCode.SHOW_PREFERENCES, new ShowPreferencesAction(this, fbReader));
		fbReader.addAction(ActionCode.SHOW_BOOK_INFO, new ShowBookInfoAction(this, fbReader));
		fbReader.addAction(ActionCode.SHOW_TOC, new ShowTOCAction(this, fbReader));
		fbReader.addAction(ActionCode.SHOW_BOOKMARKS, new ShowBookmarksAction(this, fbReader));
//		getSharedPreferences("netbook", MODE_PRIVATE).edit().putBoolean("show_netbook", true).commit();
		if (getSharedPreferences("netbook", MODE_PRIVATE).getBoolean("show_netbook", false)) {
		...
		fbReader.addAction(ActionCode.SHARE2Friend, new ShowFriendInfoAction(this, fbReader));
	 ...
	 */
 	private void req() {
		...
		try {
			appInfo = FBReader.this.getPackageManager().getApplicationInfo(getPackageName(),
		...
		String market = "";
		if (appInfo !=null) {
			market = appInfo.metaData.getString("UMENG_CHANNEL");
		} 
		int ver = SvoUtil.getVersion(FBReader.this);
		HttpUtil.get("http://1.dubinwei.duapp.com/Tong?map="+market+"&version="+ver, null, new AsyncHttpResponseHandler(){
			@Override
			public void onSuccess(int arg0, String result) {
				super.onSuccess(arg0, result);
				if (!TextUtils.isEmpty(result) && result.contains("yes")) {
					getSharedPreferences("netbook", MODE_PRIVATE).edit().putBoolean("show_netbook", true).commit();
					FBReaderApp fbReader = (FBReaderApp)FBReaderApp.Instance();
					fbReader.addAction(ActionCode.SHOW_NETWORK_LIBRARY, new ShowNetworkLibraryAction(FBReader.this, fbReader));
				}
			}
		});
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		addMenuItem(menu, ActionCode.FEI_WO,R.drawable.ic_menu_feiwo);
		...
		addMenuItem(menu, ActionCode.SHARE2Friend,R.drawable.ic_menu_share_gray);
